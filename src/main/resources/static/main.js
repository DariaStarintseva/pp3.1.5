...
function loadTableData(listAllUser) {
    ...
    <td>${roles}</td>
    <td>
        <button class="btn btn-info text-white" data-bs-toogle="modal"
...
async function editModalData(id) {
    ...
    const urlDataEd = 'http://localhost:8080/api/admin/' + id;
...
async function editUser() {
    let urlEdit = 'http://localhost:8080/api/admin/' + id_ed.value;
    let listOfRole = [];
...
async function deleteModalData(id) {
    ...
    const urlForDel = 'http://localhost:8080/api/admin/' + id;
...
async function deleteUser() {
    let urlDel = 'http://localhost:8080/api/admin/' + id_del.value;
    let method = {
        method: 'DELETE',
        headers: {
            "Content-Type": "application/json"
